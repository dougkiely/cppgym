...
// TODO: check for const-correctness in this code, although it's unlikely any code in here is const (it's all mutable data and function calls)
...
class Transform : public Attr {
    ...
    private:
        const rotation_type RAD2DEG{57.29577951308232};
...
class PolyLine : public Geom {
    ...
    protected:
        void render1() override {
            ...
            std::cout << "PolyLine::render1: glBegin: error=" << glGetError() << "\n";
            for (const auto& vertex : vertices_) {
                glVertex3f(vertex.x, vertex.y, 0.0f);
                std::cout << "PolyLine::render1: glVertex3f: error=" << glGetError() << "\n";
            }
            glEnd();
            std::cout << "PolyLine::render1: glEnd: error=" << glGetError() << "\n";
        }
...
//std::unique_ptr<Geom> make_circle(std::floating_point auto const radius=10.0f, std::unsigned_integral auto const resolution=30, const bool filled=true) {
    ...
    std::vector<point_2d<GLfloat>> vertices;
    vertices_2d vertices;

    for (uint64_t i=0; i < resolution; ++i) {
        auto angle = 2 * std::numbers::pi_v<GLfloat> * i / resolution;
        vertices.emplace_back(std::cos(angle) * radius, std::sin(angle) * radius);
    }

    /*if (filled)
        return std::make_unique<FilledPolygon>(vertices);
    else
        return std::make_unique<PolyLine>(vertices, true);*/
    ...
    return make_polygon<T>(vertices);
}
...
class Window {
    public:
        ...
        Window(std::string_view title, const std::unsigned_integral auto width, const std::unsigned_integral auto height, const std::integral auto x, const std::integral auto y) {
            ...
            //glutCloseFunc(close_func); // TODO: implement c-style callback, wonderful
        }
        ...
        ~Window() {
            if (window_is_open_) {
                ...
                // TODO: or just call glutDestroyWindow() and disregard returned error?
            }
...
class Viewer {
    public:
        ...
        void set_bounds(const std::unsigned_integral auto left, const std::unsigned_integral auto right, const std::unsigned_integral auto bottom, const std::unsigned_integral auto top) {
            ...
            transform_.set_translation({-left * scale_x, -bottom * scale_y});
            transform_.set_scale({scale_x, scale_y});
        }
    ...
    private:
        ...
        std::vector<Geom> onetime_geoms_;
