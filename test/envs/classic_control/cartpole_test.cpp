...
class Q_learning_agent {
    public:
                    ...
                    for (std::size_t k=0; k < (*Q_)[i][j].size(); ++k)
                        for (std::size_t l=0; l < (*Q_)[i][j][k].size(); ++l)
                            for (std::size_t m=0; m < (*Q_)[i][j][k][l].size(); ++m)
                                (*Q_)[i][j][k][l][m] = 0.0f; // TODO: not needed, everything's initialized to zero*/
        ...
        action_type get_action(const observation_type& obs) {
            const auto [i0, i1, i2, i3] = discretize(obs);
        ...
        void learn(const observation_type& obs, action_type action, reward_type reward, const observation_type& next_obs) {
            const auto [i0, i1, i2, i3]  = discretize(obs);
            auto& Q_actions = (*Q_)[i0][i1][i2][i3];

            const auto [next_i0, next_i1, next_i2, next_i3] = discretize(next_obs);
            const auto& Q_next_actions = (*Q_)[next_i0][next_i1][next_i2][next_i3];
        ...
        auto get_policy() const -> const std::unique_ptr<policy_type>& {

            for (std::size_t i=0; i < (*Q_).size(); ++i)
                for (std::size_t j=0; j < (*Q_)[i].size(); ++j)
                    for (std::size_t k=0; k < (*Q_)[i][j].size(); ++k)
                        for (std::size_t l=0; l < (*Q_)[i][j][k].size(); ++l)
                            (*policy_)[i][j][k][l] = argmax((*Q_)[i][j][k][l]);

            return policy_;
        }

        void step_alpha(std::integral auto step_iteration) {
            // perform epsilon-greedy action selection
            T alpha_decay_rate_{500 * alpha_min_ / static_cast<T>(step_iteration)};
            if (alpha_ > alpha_min_)
                alpha_ -= alpha_decay_rate_;
        }
    ...
    private:
        ...
        T alpha_;
        constexpr static T alpha_min_{0.1f};
        //constexpr static T alpha_decay_rate_{500 * alpha_min_ / max_steps};
        T gamma_;
        ...
        std::unique_ptr<Q_table_type> Q_{std::make_unique<Q_table_type>()}; // store Q-value of each action
        std::unique_ptr<policy_type> policy_{std::make_unique<policy_type>()};
};

template<typename A, typename E>
auto train(A& agent, E& env, const size_t max_episodes) -> const std::unique_ptr<typename A::policy_type>& {
    ...
    // policy
    return agent.get_policy(); //std::move(agent.get_policy());
}
...
auto test(A& agent, E& env, const typename A::policy_type* const policy) -> E::reward_type {
    ...
    for (size_t i=0; i < 200; ++i) {
        ...
        const auto action = (*policy)[i0][i1][i2][i3];
...
int main(int argc, char** argv) {
    ...
    constexpr T alpha_learning_rate{0.5f}; //0.05
    constexpr T gamma_discount_factor{1.0}; //0.98
    constexpr unsigned int discrete_bin_count{50}; //30
